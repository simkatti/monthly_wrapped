...
if month:
    if st.button(f"Get top 10 tracks of {month}"):
        ...
        if response.status_code == 200:
            ...
            st.subheader(
                f"You mostly listen during the {
                    data['time_of_day']} between {
                    data['time_slot']}!")

            st.space(size="small")

            st.subheader(
                f"Your total listening time in {month} is {
                    data['total_minutes']} minutes which is {
                    data['formatted_time']}")
            ...
            st.subheader(
                f"You listened {
                    data['total_artists']} different artists and {
                    data['total_songs']} different songs!")
